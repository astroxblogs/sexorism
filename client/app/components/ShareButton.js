...
const ShareButton = ({
  ...
  const quickShareLinks = useMemo(
    () => links.filter(link => ["WhatsApp", "Facebook", "Twitter", "Telegram"].includes(link.name)),
    [links]
  );
        ...
        >
          {variant === "icon" ? (
            <>
              ...
              {/* ⛔️ Hide share count on icon */}
              {/* {showCountOnIcon && <span className="text-xs font-semibold pr-1">{shareCount}</span>} */}
            </>
          ) : (
            <>
              ...
              {/* ⛔️ Hide share count pill */}
              {/* <span className="text-xs px-1.5 py-0.5 rounded-full bg-white/20 text-white">{shareCount}</span> */}
            </>
          )}
        </button>
